...
Socket.prototype.setupListeners = function () {
    ...
    this.io.on('connection', function (socket) {
        ...
        socket.on('USER:VoteByRoomId', function (options) {
            ...
            var roomUsers = this.rooms[options.user.id];
            _.each(this.rooms[options.user.id], function (user, key) {

                // I broke something down the line so we need to do user.user
                // late night coding we make stupid mistakes
                if (options.user.user.name == user.name) {
